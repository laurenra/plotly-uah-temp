...
rm(uah.raw)
# Read only the Year, Mo, Globe, Land, Ocean columns and ignore the remaining 24 columns
uah.monthly <- read.table(textConnection(uah.data), 
                          header = TRUE, 
                          colClasses = c("character", "character", rep("numeric", 3), rep("NULL",24)))
rm(uah.data)
names(uah.monthly) <- c("Year", "Month", "Global", "Land", "Ocean")
...
write.csv(uah.monthly, file = "~/Dev/Node/plotly-uah-temp/data/uah-monthly-global.csv", row.names = FALSE)
